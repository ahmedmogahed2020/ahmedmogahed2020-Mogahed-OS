...
import { openKnowledgeModal, openKnowledgeReader, editKnowledge, deleteKnowledge, knowledgeToTask, reviewKnowledge, knowledgeToGoal, knowledgeToProject, fetchKnowledgeMetadataFromForm, fetchPdfMetadataFromForm, addTimedNote, seekVideoNote, selectKnowledgeVideo, saveVideoContent, setKnowledgeFilter, setKnowledgeSearch, searchKnowledgeTag, markVideoComplete, scheduleVideoReview, videoContentToTasks, refreshKnowledgeTypeFields, changePdfPage, savePdfReadingProgress, startPdfReading, stopPdfReading, markPdfComplete, schedulePdfReview, pdfContentToTasks, refreshKnowledgeCloudFiles } from './modules/knowledge.js';
...
const actionMap = {
  ...
  'open-knowledge-modal': () => openKnowledgeModal(), 'open-knowledge-reader': id => openKnowledgeReader(id), 'edit-knowledge': id => editKnowledge(id), 'delete-knowledge': id => deleteKnowledge(id), 'knowledge-to-task': id => knowledgeToTask(id), 'review-knowledge': id => reviewKnowledge(id), 'knowledge-to-goal': id => knowledgeToGoal(id), 'knowledge-to-project': id => knowledgeToProject(id), 'fetch-knowledge-metadata': () => fetchKnowledgeMetadataFromForm(), 'fetch-pdf-metadata': () => fetchPdfMetadataFromForm(), 'add-video-note': id => addTimedNote(id), 'seek-video-note': (id, el) => seekVideoNote(id, el.dataset.noteId), 'knowledge-select-video': (id, el) => selectKnowledgeVideo(id, el.dataset.videoId), 'save-video-content': id => saveVideoContent(id), 'mark-video-complete': id => markVideoComplete(id), 'schedule-video-review': id => scheduleVideoReview(id), 'video-content-to-tasks': id => videoContentToTasks(id), 'pdf-page-next': id => changePdfPage(id, 1), 'pdf-page-prev': id => changePdfPage(id, -1), 'save-pdf-progress': id => savePdfReadingProgress(id), 'start-pdf-reading': id => startPdfReading(id), 'stop-pdf-reading': id => stopPdfReading(id), 'mark-pdf-complete': id => markPdfComplete(id), 'schedule-pdf-review': id => schedulePdfReview(id), 'pdf-content-to-tasks': id => pdfContentToTasks(id), 'refresh-knowledge-cloud-files': id => refreshKnowledgeCloudFiles(id), 'knowledge-search-tag': (_, el) => searchKnowledgeTag(el.dataset.tag), 'knowledge-filter': (_, el) => setKnowledgeFilter(el.value || el.dataset.filter || 'all'),
  ...
  'show-backup': () => showBackup(), 'show-settings': () => showSettings(), 'show-qa': () => showQA(), 'show-notifications': () => showNotifications(), 'show-guide': () => showGuide(), 'run-system-test': () => runQA(), 'export-json': () => doExport(), 'backup-date': () => doBackup(), 'force-save-data': () => forceSaveData(), 'clear-data': () => doClear(), 'drive-connect': async () => { await connectGoogleDrive(); showBackup(); }, 'drive-disconnect': () => { disconnectGoogleDrive(); showBackup(); }, 'drive-upload-now': async () => { await uploadDriveBackup().catch(() => {}); showBackup(); }, 'drive-list': async () => { await listDriveBackups(); showBackup(); }, 'drive-restore-latest': () => restoreLatestDriveBackup(), 'cloud-sign-in': () => cloudSignIn(), 'cloud-sign-up': () => cloudSignUp(), 'cloud-sign-out': () => cloudSignOut(), 'cloud-upload': () => cloudUpload(), 'cloud-download': () => cloudDownload(), 'cloud-merge': () => cloudMerge(), 'cloud-status': () => cloudStatus(), 'migrate-knowledge-files': () => migrateKnowledgeFilesToCloud(),
...
function handleChange(event) {
  ...
  if (el.matches('[data-action="drive-history"]')) updateDriveKeepHistory(el.checked);
  if (el.matches('[data-action="backend-provider"]')) updateBackendProvider(el.value);
  if (el.matches('[data-action="backend-sync-mode"]')) updateBackendSyncMode(el.value);
  if (el.matches('[data-action="backend-file-storage"]')) updateBackendFileStorage(el.value);
  if (el.matches('[data-action="backend-storage-bucket"]')) updateBackendStorageBucket(el.value);
  if (el.matches('[data-action="backend-enabled"]')) updateBackendEnabled(el.value);
  if (el.matches('[data-action="backend-auto-sync"]')) updateBackendAutoSync(el.value);
}
...
function handleInput(event) {
  ...
  if (el.matches('[data-action="backend-anon-key"]')) updateBackendAnonKey(el.value);
}
