...
export function settingsPanel() {
  return `<article class="card"><h3>الإعدادات</h3><div class="setting-row"><div><b>اسم المستخدم</b><p class="meta">يظهر في الرئيسية</p></div><input style="max-width:220px" value="${safeText(appState.data.settings.userName||'مجاهد')}" data-action="settings-name"></div><div class="setting-row"><div><b>YouTube API Key</b><p class="meta">يُستخدم مرة واحدة داخل المشروع لجلب بيانات الفيديو والبلاي ليست تلقائيًا.</p></div><input style="max-width:320px" type="password" value="${safeText(appState.data.settings.youtubeApiKey||'')}" placeholder="AIza..." data-action="settings-youtube-key"></div><div class="setting-row"><div><b>الحفظ التلقائي</b><p class="meta">مفعل دائمًا في هذه النسخة</p></div><span class="badge success">مفعل</span></div></article>`;
}
...
export function updateYouTubeApiKey(value) { appState.data.settings.youtubeApiKey = value.trim(); localStorage.setItem('mogahed_os_data_v1', JSON.stringify(appState.data)); }
