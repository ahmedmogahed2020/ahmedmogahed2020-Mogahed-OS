// Extracted from V83 inline script block 27. Original id: mogahed-v82-hardening-cleanup-script
...
(function () {
  ...
  function notify(msg){ try { if(window.MogahedOSX && MogahedOSX.toast) return MogahedOSX.toast(msg); } catch(e){} try { console.log("Mogahed OS:", msg); } catch(e){} }
  ...
  function patchInsertAdjacentHTML(){
    try {
      // V82.1 Regression Fix: do NOT patch Element.prototype globally.
      // Global sanitizing of every inserted fragment can break legacy plugins that intentionally use inline handlers.
      // Sanitizing remains scoped to openModal/user-controlled HTML plus URL validation.
  ...
  function registerExternalActionNoops(){
    try {
      ...
      [
        ...
        if(!A[name]) A[name] = function(){ /* handled by version-specific delegated listener */ };
      });
